...
def detect_color_clusters(image, color_range=((0, 160), (200, 250), (0, 120)),min_area=21):
    ...
    min_contour_area = min_area  # Minimum number of pixels for a contour to be considered a "cluster"
    ...
    for cnt in contours:
        ...
        # Check if there are larger bounding boxes that contain this box
        is_inside = False
        ...
        # If not inside any larger box, add the bounding box to the list
        if not is_inside:
...
def ocr_only(screenshot):
    # Enhance color difference using CLAHE
    ...
    # Use Tesseract to do OCR on the binarized image
    text = pytesseract.image_to_string(binary_image, config='--psm 6 -l chi_sim+eng')
...
def findocr(image, color_range=((0, 160), (200, 250), (0, 120)), max_x_diff=25, max_y_diff=150):
    ...
    for x, y, w, h in bounding_boxes:
        ...
        if ocr_text.strip() and any(char.isalnum() for char in ocr_text):
            ...
            # Draw a bounding box around the region where OCR was performed
...
def replace_content(match):
    return f"Celsia: {match.group(1)}"

def dbfindocr(image, color_range=((0, 160), (200, 250), (0, 120)), max_x_diff=25, max_y_diff=150):
    image = cv2.resize(image, None, fx=2, fy=2, interpolation=cv2.INTER_CUBIC)
...
def convert_timestamp(timestamp):
    # Convert default format to desired format
    desired_format = datetime.strptime(timestamp, "%a %b %d %H:%M:%S %Y").strftime("%Y-%m-%d %H:%M:%S")
...
def wait_until_finish(x=0, y=0, width=0, height=0, threshold=10, max_wait=60):
    if x == y == width == height == 0:
        width, height = pyautogui.size()  # Get screen dimensions
        monitor_region = (0, 0, width, height)
    else:
        monitor_region = (x, y, width, height)
    ...
    while max_wait > 0:
        ...
        # Calculate the average movement value
        avg_movement = np.sum(diff_image) / (width * height)

        if avg_movement <= threshold:
            return True  # Movement is below threshold, return True
        
        time.sleep(1)  # Wait for a second before checking again
        max_wait -= 1
        reference_screen = current_screen
    
    return False  # Maximum wait time reached, return False
        ...
        if command.startswith("move_mouse"):
            ...
            if len(args) == 1:
                ...
                if len(args) == 2:
                    ...
                    threshold = float(args[0]) if len(args) > 0 else 10
                    max_wait = float(args[1]) if len(args) > 1 else 10
                    wait_until_loaded(threshold=threshold, max_wait=max_wait)
                else:
                    threshold = float(args[0]) if len(args) > 0 else 10
                    max_wait = float(args[1]) if len(args) > 1 else 10
                    wait_until_finish(threshold=threshold, max_wait=max_wait)

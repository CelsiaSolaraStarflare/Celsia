...
def capture_window_with_index(index,application):
    ...
    # Open the captured image using PIL and return it
    captured_image = Image.open(output_path)
...
def GUT(query):
    query_message = re.split(r'[.,]', query_message)
...
def N(commands):
    ...
    for command in commands:
        if command.startswith("move_mouse"):
            ...
            try:
                index,application = map(str, command.split()[1:])
            except Exception as e:
                application = map(str, command.split()[1:])
                index = 1
            if index == 0:
                index = 1
            imgkept = capture_window_with_index(index,application)

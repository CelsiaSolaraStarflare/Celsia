...
def findocr(image, color_range=((0, 160), (200, 250), (0, 120)), model_file=False, max_x_diff=25, max_y_diff=150, bg_color=None):
    ...
    for i in range(ocr_results.count('')):
        ocr_results.pop(ocr_results.index(''))
...
def OCR2(model_file="contours.joblib",screenshot=None):
    # Debugging code: print the value of the screenshot variable
...
def wait_until_loaded(threshold=10, interval=1, max_wait=50):
    prev_screenshot = None
    while max_wait > 0:
        # Take a screenshot
        screenshot = ImageGrab.grab()
        if prev_screenshot is not None:
            # Compare the current and previous screenshots
            diff = ImageChops.difference(screenshot, prev_screenshot)
            num_changed_pixels = sum(diff.histogram())
            if num_changed_pixels <= threshold:
                # The screen has stopped changing
                break
        # Update the previous screenshot and wait for the next iteration
    ...
    for command in commands:
        if command.startswith("move_mouse"):
            ...
            if len(args) == 1:
                ...
                interval = float(args[1]) if len(args) > 1 else 1
                max_wait = float(args[2]) if len(args) > 2 else 10
                wait_until_loaded(threshold, interval, max_wait)
